...
`timeit` modules. to measure wall or CPU time for pieces of code.
...
Profiling of C programs can be done by compiling (and linking) code with
the `-pg` option, running the resulting executable to collect information on
program execution, then using `gprof` to analyze the collected data.
